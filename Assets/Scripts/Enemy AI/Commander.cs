...
// Class for the commander enemy
// This code manages the logic of the commander enemy.
public class Commander : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        enemyAI.health = 8; // Set the health to 8
        ...
        ability = Resources.Load("Enemy Abilities/Laser2"); // Instantiate the "Laser1" object and set it as the ability
    }
    ...
    public IEnumerator TurnActions()
    {
        ...
        Collider2D[] hitBoxes = transform.GetChild(0).GetComponentsInChildren<Collider2D>(); // Create and set the hitboxes to the first child component's children Colliders
        bool hitBoxTouching = false; // Stores a bool deciding if the hit box is touching or not
        foreach (Collider2D hitBox in hitBoxes) 
        { // For each hit box in hit boxes,
            if (hitBox.IsTouching(enemyAI.player.GetComponent<Collider2D>()))
            { // If the hit boxes are touching the player,
                hitBoxTouching = true; // Set hit box touching to true
        ...
        if (!hitBoxTouching)
        { // If the hit boxes are not touching the player,
